...
function createTransform(mode, size) {
  ...
  control.addEventListener('objectChange', () => { syncDirectionalTargets(); updateModelBasis(); if(control.object===modelRoot)syncOrbitPivotToModel(true); renderInspector(); updateSelectionBox(); });
...
function restorePreviewTransform(){if(!state.previewStartTransform)return false;modelRoot.position.copy(state.previewStartTransform.position);modelRoot.quaternion.copy(state.previewStartTransform.quaternion);modelRoot.scale.copy(state.previewStartTransform.scale);updateModelBasis();syncOrbitPivotToModel();return true;}
...
function syncOrbitPivotToModel(keepModelCentered=false){const pivot=modelPivot(),offset=pivot.clone().sub(controls.target);if(keepModelCentered&&offset.lengthSq()>0)camera.position.add(offset);controls.target.copy(pivot);camera.lookAt(pivot);controls.update();}
