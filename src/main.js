...
function removeWireframeLines(object){object?.traverse(child=>{if(!child.isMesh||!child.userData.wireframeLines)return;child.remove(child.userData.wireframeLines);child.userData.wireframeLines.geometry.dispose();child.userData.wireframeLines.material.dispose();delete child.userData.wireframeLines;});}
function rebuildWireframe(){
  ...
  state.currentModel.traverse(child=>{
    ...
    const geometry=topologyLineGeometry(child.geometry);if(!geometry)return;
    const lines=new THREE.LineSegments(geometry,new THREE.LineBasicMaterial({color:'#111318',transparent:true,opacity:state.wireOpacity,linewidth:state.wireWidth,depthTest:true,depthWrite:false,polygonOffset:true,polygonOffsetFactor:-1,polygonOffsetUnits:-1}));
    lines.name='Wireframe';lines.renderOrder=2;lines.frustumCulled=false;child.add(lines);child.userData.wireframeLines=lines;
  });
}
...
function createLight(type='spot',options={}) {
  ...
  const names={spot:'???',point:'???',directional:'???',ambient:'???',rect:'??'};
...
function applyPreset(preset){clearLights();scene.background=null;preset.lights.forEach(config=>createLight(config.type,{...config,deletable:false}));state.presetName=preset.name;clearSelection();renderPresets();toast('???????',preset.name);}
...
function renderInspector(){const object=state.selectedObject;if(!object){dom.inspector.innerHTML='<span>请选择模型或灯光</span>';return;}if(!object.isLight){dom.inspector.innerHTML=`<h3>◫ 当前模型</h3><label>旋转 Y<input class="range" id="modelRotation" type="range" min="-180" max="180" value="${THREE.MathUtils.radToDeg(modelRoot.rotation.y)}"></label><label>缩放<input class="range" id="modelScale" type="range" min="20" max="180" value="${modelRoot.scale.x*100}"></label><div class="coord-grid"><label>X<input class="number-input model-pos" data-axis="x" value="${modelRoot.position.x.toFixed(1)}"></label><label>Y<input class="number-input model-pos" data-axis="y" value="${modelRoot.position.y.toFixed(1)}"></label><label>Z<input class="number-input model-pos" data-axis="z" value="${modelRoot.position.z.toFixed(1)}"></label></div>`;$('#modelRotation').oninput=e=>{modelRoot.rotation.y=THREE.MathUtils.degToRad(Number(e.target.value));updateSelectionBox();};$('#modelScale').oninput=e=>{modelRoot.scale.setScalar(Number(e.target.value)/100);updateSelectionBox();};$('.model-pos').forEach(input=>input.oninput=e=>{const value=Number(e.target.value);if(!Number.isFinite(value))return;modelRoot.position[e.target.dataset.axis]=value;updateModelBasis();syncOrbitPivotToModel(true);updateSelectionBox();});return;}
  const typeName={spot:'聚光灯',point:'点光源',directional:'方向光',ambient:'环境光'}[object.userData.type],position=object.position,rotation=object.rotation;
  dom.inspector.innerHTML=`<h3><span class="tree-dot"></span><input id="lightName" class="inline-name" value="${object.name}"><button id="deleteLight">删除</button></h3><label>类型<select disabled><option>${typeName}</option></select></label><label>显示状态<select id="lightVisible"><option value="on" ${object.userData.enabled?'selected':''}>显示</option><option value="off" ${!object.userData.enabled?'selected':''}>隐藏</option></select></label><label>亮度 <b id="levelValue">${object.userData.level}%</b><input class="range" id="lightLevel" type="range" min="0" max="100" value="${object.userData.level}"></label><label>颜色<input id="lightColor" type="color" value="#${object.color.getHexString()}"></label>${object.userData.type==='spot'?`<label>照射角度 <b id="angleValue">${Math.round(THREE.MathUtils.radToDeg(object.angle))}°</b><input class="range" id="lightAngle" type="range" min="5" max="100" value="${THREE.MathUtils.radToDeg(object.angle)}"></label>`:''}${object.userData.type!=='ambient'?`<div class="coord-grid">${['x','y','z'].map(axis=>`<label>${axis.toUpperCase()}<input class="number-input coord" data-axis="${axis}" value="${position[axis].toFixed(1)}"></label>`).join('')}</div><div class="coord-grid">${['x','y','z'].map(axis=>`<label>R${axis.toUpperCase()}<input class="number-input rotation" data-axis="${axis}" value="${THREE.MathUtils.radToDeg(rotation[axis]).toFixed(0)}"></label>`).join('')}</div>`:''}`;
  $('#lightName').onchange=e=>{object.name=e.target.value.trim()||typeName;renderSceneTree();};$('#lightVisible').onchange=e=>{object.userData.enabled=e.target.value==='on';object.visible=object.userData.enabled;const entry=state.lights.find(item=>item.light===object);if(entry?.helper)entry.helper.visible=object.userData.enabled&&state.selectedObject===object;renderSceneTree();};$('#lightLevel').oninput=e=>{object.userData.level=Number(e.target.value);object.intensity=actualIntensity(object.userData.type,object.userData.level);$('#levelValue').textContent=`${e.target.value}%`;};$('#lightColor').oninput=e=>object.color.set(e.target.value);if($('#lightAngle'))$('#lightAngle').oninput=e=>{object.angle=THREE.MathUtils.degToRad(Number(e.target.value));$('#angleValue').textContent=`${e.target.value}°`;};$$('.coord').forEach(input=>input.onchange=e=>{object.position[e.target.dataset.axis]=Number(e.target.value);syncDirectionalTarget(object);});$$('.rotation').forEach(input=>input.onchange=e=>{object.rotation[e.target.dataset.axis]=THREE.MathUtils.degToRad(Number(e.target.value));syncDirectionalTarget(object);});$('#deleteLight').onclick=()=>deleteLight(object); if(object.userData.deletable===false)$('#deleteLight').disabled=true;
}
...
function renderPresets(){
  ...
  presets.forEach((preset,index)=>{
    ...
    const swatch=document.createElement('span');swatch.className='swatch';swatch.style.background=preset.background||'#242936';
    const content=document.createElement('div');const name=document.createElement('strong');name.textContent=preset.name;const description=document.createElement('small');description.textContent=preset.description||'???';content.append(name,description);
    const selected=document.createElement('em');selected.textContent=state.presetName===preset.name?'?':'';apply.append(swatch,content,selected);apply.onclick=()=>applyPreset(preset);card.append(apply);
    if(index>=builtInPresets.length){const actions=document.createElement('div');actions.className='preset-card-actions';[['edit','??'],['export','??'],['delete','??']].forEach(([action,label])=>{const button=document.createElement('button');button.type='button';button.textContent=label;button.onclick=event=>{event.stopPropagation();if(action==='delete'){state.customPresets.splice(index-builtInPresets.length,1);persistPresets();renderPresets();return;}if(action==='edit'){const nextName=prompt('????',preset.name);if(!nextName)return;const nextDescription=prompt('?????????',preset.description||'');Object.assign(preset,{name:nextName.trim()||preset.name,description:nextDescription});persistPresets();renderPresets();return;}downloadJson(preset,preset.name+'.json');};actions.append(button);});card.append(actions);}
...
async function parseModel(file,resourceFiles=[]){
  ...
  if(extension==='obj'){const loader=new OBJLoader(manager),mtlFile=resourceFiles.find(item=>/\.mtl$/i.test(item.name));if(mtlFile){const materials=new MTLLoader(manager).parse(await mtlFile.text(),'');materials.preload();loader.setMaterials(materials);}return configureImportedObject(loader.parse(await file.text()));}
...
async function uploadModels(files){
  ...
  for(const file of modelFiles){const placeholder=addAsset(file.name.replace(/\.[^.]+$/,''),new THREE.Group(),'...',false);placeholder.imported=true;placeholder.status='\u89e3\u6790\u4e2d';renderAssets();try{placeholder.object=await parseModel(file,[...state.textureFiles,...resources]);placeholder.icon='\u25eb';placeholder.status=(file.size/1024/1024).toFixed(1)+' MB \u00b7 \u5df2\u5c31\u7eea';openAsset(placeholder);}catch(error){placeholder.status='\u52a0\u8f7d\u5931\u8d25\uff1a'+error.message;placeholder.icon='!';}renderAssets();}
  if(!modelFiles.length&&resources.length)toast('\u8d44\u6e90\u6587\u4ef6\u5df2\u52a0\u5165','\u8bf7\u5c06\u6a21\u578b\u4e0e\u8d34\u56fe\u6587\u4ef6\u4e00\u8d77\u91cd\u65b0\u9009\u62e9');
}
...
function mimeType(){return['video/webm;codecs=vp9','video/webm;codecs=vp8','video/webm'].find(type=>MediaRecorder.isTypeSupported(type))||'';}
...
async function recordAsset(asset,batch=false){
  ...
  const stream=recordingRenderer.domElement.captureStream(state.fps),recorder=new MediaRecorder(stream,{mimeType:mimeType(),videoBitsPerSecond:8_000_000}),chunks=[];
...
state.lights.forEach(({light})=>{if(light.userData.previewIntensity===undefined)light.userData.previewIntensity=light.intensity;light.intensity=light.userData.previewIntensity;light.visible=true;});lightHelpers.visible=false;grid.visible=false;worldAxes.visible=false;selectionBox.visible=false;controls.enabled=false;
  state.recording={start:performance.now(),duration,renderer:recordingRenderer,camera:recordCamera,recorder,stopping:false,rotate:$('#rotationMode').value==='single',visibility,savedTransform,rotationOffset:new THREE.Quaternion(),stopTimer:null};
  ...
  try{recorder.start(100);state.recording.stopTimer=setTimeout(()=>{if(state.recording&&!state.recording.stopping){state.recording.stopping=true;recorder.stop();}},duration+60);return await result;}
  finally{
    if(state.recording?.stopTimer)clearTimeout(state.recording.stopTimer);dom.workspace.classList.remove('recording-active');state.recording=null;modelRoot.position.copy(savedTransform.position);modelRoot.quaternion.copy(savedTransform.quaternion);modelRoot.scale.copy(savedTransform.scale);updateModelBasis();
state.lights.forEach(({light},index)=>{if(visibility.lightIntensities[index]!==undefined)light.intensity=visibility.lightIntensities[index];if(visibility.lightVisibility[index]!==undefined)light.visible=visibility.lightVisibility[index];});lightHelpers.visible=visibility.helpers;
    scene.background=previousBackground;recordingRenderer.dispose();refreshTransforms();renderSceneTree();
  }
}
...
function animate(time){requestAnimationFrame(animate);controls.update();state.frames++;if(time-state.fpsTime>700){const fps=$('#fpsText');if(fps)fps.textContent=`${Math.round(state.frames*1000/(time-state.fpsTime))} FPS`;state.frames=0;state.fpsTime=time;}if(state.frames%3===0&&state.previewEnabled)state.lights.forEach(({helper})=>helper?.update?.());updateViewGizmo();if(state.previewPlaying&&!state.recording){const duration=Number(dom.duration.value)*1000;state.previewProgress=(state.previewProgress+Math.max(0,time-state.previewLastTime)/duration)%1;state.previewLastTime=time;const base=previewTransform(),rotation=new THREE.Quaternion().setFromAxisAngle(THREE.Object3D.DEFAULT_UP,state.previewProgress*Math.PI*2);modelRoot.quaternion.copy(base.quaternion).multiply(rotation);modelRoot.position.copy(base.position);modelRoot.scale.copy(base.scale);updateModelBasis();updateTimelineUI();}else{state.previewLastTime=time;}if(state.recording){const task=state.recording,progress=Math.min((time-task.start)/task.duration,1);if(task.rotate){task.rotationOffset.setFromAxisAngle(THREE.Object3D.DEFAULT_UP,progress*Math.PI*2);modelRoot.quaternion.copy(task.savedTransform.quaternion).multiply(task.rotationOffset);}task.renderer.render(scene,task.camera);if(progress>=1&&!task.stopping){task.stopping=true;setTimeout(()=>task.recorder.stop(),80);}}renderer.render(scene,camera);} 
requestAnimationFrame(animate);

$('#sceneSearch').addEventListener('input',renderSceneTree);
dom.input.addEventListener('change',event=>uploadModels([...event.target.files]));
...
function setupInspectorResize(){
  ...
  dom.inspector.addEventListener('pointermove',event=>{if(!resizeState)return;const next=THREE.MathUtils.clamp(resizeState.startHeight+(resizeState.startY-event.clientY),240,Math.max(300,window.innerHeight-220));dom.inspector.style.flexBasis=next+'px';dom.inspector.style.height=next+'px';});
