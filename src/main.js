...
async function addDownload(name,blob){const filename=`${name}.webm`,url=URL.createObjectURL(blob),link=document.createElement('a');link.href=url;link.download=filename;link.textContent=`Download ${filename}`;dom.downloads.append(link);if(state.historyFolderHandle){try{const fileHandle=await state.historyFolderHandle.getFileHandle(filename,{create:true}),writer=await fileHandle.createWritable();await writer.write(blob);await writer.close();toast('已保存到历史文件夹',filename);setTimeout(()=>URL.revokeObjectURL(url),60000);return url;}catch(error){console.warn('Unable to save to history folder',error);toast('历史文件夹保存失败','已改为浏览器下载');}}link.click();setTimeout(()=>URL.revokeObjectURL(url),60000);return url;}
async function recordAsset(asset,batch=false){
  if(asset.id!==state.currentAssetId)loadAsset(asset);
  await new Promise(resolve=>setTimeout(resolve,160));
  const savedTransform={position:modelRoot.position.clone(),quaternion:modelPivotGroup.quaternion.clone(),scale:modelPivotGroup.scale.clone()},[width,height]=ratioConfig(),viewport=dom.root.getBoundingClientRect(),frameRect=dom.frame.getBoundingClientRect(),fullWidth=dom.root.clientWidth,fullHeight=dom.root.clientHeight,cropX=Math.max(0,Math.min(fullWidth-1,frameRect.left-viewport.left)),cropY=Math.max(0,Math.min(fullHeight-1,frameRect.top-viewport.top)),cropWidth=Math.max(1,Math.min(fullWidth-cropX,frameRect.width)),cropHeight=Math.max(1,Math.min(fullHeight-cropY,frameRect.height)),duration=Number(dom.duration.value)*1000;
...
async function startSingleRecording(){if(state.recording||state.batchRunning)return;if(state.recordStage==='idle'){enterFraming();return;}const asset=state.assets.find(a=>a.id===state.currentAssetId);if(!asset)return toast('\u8bf7\u5148\u9009\u62e9\u6a21\u578b','\u4ece\u53f3\u4fa7\u8d44\u4ea7\u5217\u8868\u9009\u62e9\u4e00\u4e2a\u6a21\u578b');try{const blob=await recordAsset(asset);await addDownload(`${asset.name}_${state.presetName}_${dom.ratio.value.replace(':','x')}`,blob);toast('\u5f55\u5236\u5b8c\u6210',`${asset.name} \u5df2\u5b8c\u6210 360\u00b0 \u89c6\u9891\u5f55\u5236`);}catch(error){toast('\u5f55\u5236\u5931\u8d25',error.message);}finally{exitFraming();}}
async function startBatch(){if(state.batchRunning||state.recording)return;const assets=state.assets.filter(a=>a.selected&&a.object);if(!assets.length)return toast('\u6ca1\u6709\u6279\u91cf\u4efb\u52a1','\u8bf7\u5148\u52fe\u9009\u81f3\u5c11\u4e00\u4e2a\u6a21\u578b');state.batchRunning=true;dom.downloads.innerHTML='';dom.queueDone.textContent='0';dom.queueProgress.style.width='0';dom.queueStatus.textContent='\u6e32\u67d3\u4e2d';for(let index=0;index<assets.length;index++){dom.queueEta.textContent=`\u6b63\u5728\u5904\u7406 ${assets[index].name}`;try{const blob=await recordAsset(assets[index],true);await addDownload(`${assets[index].name}_${state.presetName}_${dom.ratio.value.replace(':','x')}`,blob);}catch(error){console.error(error);}dom.queueDone.textContent=String(index+1);dom.queueProgress.style.width=`${(index+1)/assets.length*100}%`;}state.batchRunning=false;dom.queueStatus.textContent='\u5168\u90e8\u5b8c\u6210';dom.queueEta.textContent='\u89c6\u9891\u53ef\u4ee5\u4e0b\u8f7d';exitFraming();toast('\u6279\u91cf\u6e32\u67d3\u5b8c\u6210',`\u5df2\u751f\u6210 ${assets.length} \u4e2a\u89c6\u9891\u6587\u4ef6`);}
...
async function selectHistoryFolder(){try{if('showDirectoryPicker' in window){const handle=await window.showDirectoryPicker({mode:'readwrite'});state.historyFolderHandle=handle;updateHistoryFolderStatus(handle.name);toast('历史文件夹已选择','后续录制文件会保存到该文件夹');return;}dom.historyFolderInput.click();}catch(error){if(error?.name!=='AbortError')toast('无法选择历史文件夹',error.message||'请重试');}}
dom.historyFolderButton.addEventListener('click',selectHistoryFolder);
dom.historyFolderInput.addEventListener('change',event=>{const file=event.target.files?.[0];if(!file)return;state.historyFolderHandle=null;updateHistoryFolderStatus(file.webkitRelativePath.split('/')[0]||'已选择文件夹');toast('历史文件夹已选择','当前浏览器将继续使用下载方式保存文件');event.target.value='';});
