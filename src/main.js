import './styles.css';
function captureSceneState(){return{background:scene.background?.isColor?`#${scene.background.getHexString()}`:'studio-gradient',model:{position:modelRoot.position.toArray(),quaternion:modelPivotGroup.quaternion.toArray(),scale:modelPivotGroup.scale.toArray()},lights:state.lights.map(({light})=>({type:light.userData.type,name:light.name,color:'#'+light.color.getHexString(),level:light.userData.level,position:light.position.toArray(),rotation:[light.rotation.x,light.rotation.y,light.rotation.z],angle:light.angle,width:light.userData.width,height:light.userData.height,enabled:light.visible}))};}
...
const dom = {
  ...
  queueDone: $('#queueDone'), queueTotal: $('#queueTotal'), queueEta: $('#queueEta'), downloads: $('#downloadList'), historyFolderButton: $('#historyFolderButton'), historyFolderInput: $('#historyFolderInput'), historyFolderStatus: $('#historyFolderStatus'),
...
function createTransform(mode, size) {
  ...
  control.addEventListener('dragging-changed', event => {
    ...
    if(!event.value&&control.object===modelRoot)syncOrbitPivotToModel();
  });
  control.addEventListener('objectChange', () => { syncDirectionalTargets(); updateModelBasis(); if(control.object===modelRoot)syncOrbitPivotToModel(); renderInspector(); updateSelectionBox(); });
...
const modelRoot = new THREE.Group(); const modelPivotGroup = new THREE.Group(); modelRoot.add(modelPivotGroup); scene.add(modelRoot);
...
const state = {
  ...
  frame:{ x:.5, y:.49, scale:1 }, previewStartTransform:null, wireOverlay:false, wireMode:'feature', wireWidth:1, wireOpacity:.8, previewQuality:null, history:[], redo:[], frames:0, fpsTime:performance.now(), historyFolderHandle:null, historyFolderName:''
};
...
function loadAsset(asset) {
  if(!asset?.object)return;saveCurrentCameraState();if(asset.imported){state.displayMode='material';syncDisplayModeButtons();}detachTransforms();modelRoot.visible=true;modelPivotGroup.clear();const object=prepareAssetInstance(asset),normalized=new THREE.Box3().setFromObject(object);
  modelRoot.position.set(0,-normalized.min.y,0); modelRoot.rotation.set(0,0,0); modelRoot.scale.set(1,1,1); modelPivotGroup.rotation.set(0,0,0); modelPivotGroup.scale.set(1,1,1); modelPivotGroup.add(object);
...
function restorePreviewTransform(){if(!state.previewStartTransform)return false;modelRoot.position.copy(state.previewStartTransform.position);modelPivotGroup.quaternion.copy(state.previewStartTransform.quaternion);modelPivotGroup.scale.copy(state.previewStartTransform.scale);updateModelBasis();syncOrbitPivotToModel();return true;}
function resetCameraView(){const pivot=modelPivot(),box=modelBounds(),distance=cameraFitDistance(box,2.1),front=new THREE.Vector3(0,0,1).applyQuaternion(modelPivotGroup.quaternion).normalize(),up=new THREE.Vector3(0,1,0).applyQuaternion(modelPivotGroup.quaternion).normalize();clearCameraPan();camera.up.copy(up);controls.target.copy(pivot);camera.position.copy(pivot).addScaledVector(front,distance);camera.lookAt(pivot);camera.near=Math.max(.01,distance/1000);camera.far=Math.max(200,distance*30);camera.updateProjectionMatrix();updateModelBasis();controls.update();}
...
function renderInspector(){
  ...
  if(!object.isLight){
    dom.inspector.innerHTML=`<h3>\u6a21\u578b\u8c03\u8282</h3><label>Y \u8f74\u65cb\u8f6c<input class="range" id="modelRotation" type="range" min="-180" max="180" value="${THREE.MathUtils.radToDeg(modelPivotGroup.rotation.y)}"></label><label>\u6a21\u578b\u7f29\u653e<input class="range" id="modelScale" type="range" min="20" max="180" value="${modelPivotGroup.scale.x*100}"></label><div class="coord-grid"><label>X<input class="number-input model-pos" data-axis="x" value="${modelRoot.position.x.toFixed(1)}"></label><label>Y<input class="number-input model-pos" data-axis="y" value="${modelRoot.position.y.toFixed(1)}"></label><label>Z<input class="number-input model-pos" data-axis="z" value="${modelRoot.position.z.toFixed(1)}"></label></div>`;
    $('#modelRotation').oninput=e=>{modelPivotGroup.rotation.y=THREE.MathUtils.degToRad(Number(e.target.value));updateModelBasis();updateSelectionBox();};
    $('#modelScale').oninput=e=>{modelPivotGroup.scale.setScalar(Number(e.target.value)/100);updateModelBasis();updateSelectionBox();};
    $('.model-pos').forEach(input=>input.oninput=e=>{const value=Number(e.target.value);if(!Number.isFinite(value))return;modelRoot.position[e.target.dataset.axis]=value;updateModelBasis();syncOrbitPivotToModel();updateSelectionBox();});
    return;
  }
...
function safeRenderName(value){return String(value||'未命名').trim().replace(/[<>:"/\|?*\u0000-]/g,'-').replace(/\s+/g,' ').replace(/[. ]+$/,'')||'未命名';}
async function ensureHistoryFolder(){const handle=state.historyFolderHandle;if(!handle){toast('请先选择历史文件夹','请选择可写入的视频保存位置');return false;}try{if(await handle.queryPermission({mode:'readwrite'})!=='granted'){const permission=await handle.requestPermission({mode:'readwrite'});if(permission!=='granted')throw new Error('未授予文件夹写入权限');}return true;}catch(error){state.historyFolderHandle=null;updateHistoryFolderStatus();toast('历史文件夹不可写','请重新选择并授予写入权限');return false;}}
async function nextRenderFolder(asset){const root=state.historyFolderHandle;let highest=0;for await(const [name,entry] of root.entries()){if(entry.kind!=='directory')continue;const match=/^(\d+)-/.exec(name);if(match)highest=Math.max(highest,Number(match[1]));}const folderName=String(highest+1).padStart(3,'0')+'-'+safeRenderName(asset.name)+'-'+safeRenderName(state.presetName)+'-'+safeRenderName(dom.ratio.value.replace(':','x'));const folder=await root.getDirectoryHandle(folderName,{create:true});return{folder,name:folderName};}
async function createRenderOutput(asset){if(!(await ensureHistoryFolder()))throw new Error('未选择可写的历史文件夹');return nextRenderFolder(asset);}
async function saveRenderedVideo(output,blob){const fileHandle=await output.folder.getFileHandle(output.name+'.webm',{create:true}),writer=await fileHandle.createWritable();try{await writer.write(blob);}finally{await writer.close();}const item=document.createElement('span');item.textContent='已保存：'+output.name;dom.downloads.append(item);return output.name;}
async function videoEncodingConfig(width,height,fps){if(!globalThis.VideoEncoder||!globalThis.VideoFrame)throw new Error('当前浏览器不支持精确视频编码，请使用最新版 Chrome 或 Edge');for(const candidate of [{codec:'vp09.00.10.08',containerCodec:'V_VP9'},{codec:'vp8',containerCodec:'V_VP8'}]){const config={codec:candidate.codec,width,height,bitrate:12000000,framerate:fps,latencyMode:'quality'};if((await VideoEncoder.isConfigSupported(config)).supported)return{...config,containerCodec:candidate.containerCodec};}throw new Error('当前浏览器不支持 VP8/VP9 精确视频编码');}
async function encodeRecordingFrames(task){const frameDuration=Math.round(1000000/task.fps),lastFrame=task.totalFrames;for(let frame=0;frame<=lastFrame;frame++){const progress=lastFrame===0?1:frame/lastFrame;if(task.rotate){task.rotationOffset.setFromAxisAngle(THREE.Object3D.DEFAULT_UP,progress*Math.PI*2);modelPivotGroup.quaternion.copy(task.savedTransform.quaternion).multiply(task.rotationOffset);}task.renderer.render(scene,task.camera);const timestamp=Math.round(frame*1000000/task.fps),videoFrame=new VideoFrame(task.renderer.domElement,{timestamp,duration:frameDuration});task.encoder.encode(videoFrame,{keyFrame:frame%task.fps===0});videoFrame.close();if(task.encoder.encodeQueueSize>2)await task.encoder.flush();if(frame%2===1)await new Promise(resolve=>setTimeout(resolve,0));}}
async function recordAsset(asset,batch=false){if(asset.id!==state.currentAssetId)loadAsset(asset);await new Promise(resolve=>setTimeout(resolve,160));const savedTransform={position:modelRoot.position.clone(),quaternion:modelPivotGroup.quaternion.clone(),scale:modelPivotGroup.scale.clone()},[width,height]=ratioConfig(),viewport=dom.root.getBoundingClientRect(),frameRect=dom.frame.getBoundingClientRect(),fullWidth=dom.root.clientWidth,fullHeight=dom.root.clientHeight,cropX=Math.max(0,Math.min(fullWidth-1,frameRect.left-viewport.left)),cropY=Math.max(0,Math.min(fullHeight-1,frameRect.top-viewport.top)),cropWidth=Math.max(1,Math.min(fullWidth-cropX,frameRect.width)),cropHeight=Math.max(1,Math.min(fullHeight-cropY,frameRect.height)),duration=Number(dom.duration.value)*1000;const recordingRenderer=new THREE.WebGLRenderer({antialias:true,alpha:false,preserveDrawingBuffer:true,powerPreference:'high-performance'});recordingRenderer.setClearColor(0x0e0e10,1);const previousBackground=scene.background;scene.background=backdropTexture;recordingRenderer.setSize(width,height,false);recordingRenderer.setPixelRatio(1);recordingRenderer.shadowMap.enabled=true;recordingRenderer.shadowMap.type=THREE.PCFSoftShadowMap;recordingRenderer.outputColorSpace=THREE.SRGBColorSpace;recordingRenderer.toneMapping=renderer.toneMapping;recordingRenderer.toneMappingExposure=renderer.toneMappingExposure;const recordCamera=camera.clone();recordCamera.clearViewOffset();recordCamera.aspect=fullWidth/fullHeight;recordCamera.setViewOffset(fullWidth,fullHeight,cropX,cropY,cropWidth,cropHeight);recordCamera.updateProjectionMatrix();recordCamera.updateMatrixWorld();const config=await videoEncodingConfig(width,height,state.fps),target=new ArrayBufferTarget(),muxer=new Muxer({target,video:{codec:config.containerCodec,width,height,frameRate:state.fps}}),encodingError={value:null},encoder=new VideoEncoder({output:(chunk,meta)=>muxer.addVideoChunk(chunk,meta),error:error=>{encodingError.value=error;}});encoder.configure(config);const visibility={helpers:lightHelpers.visible,grid:grid.visible,axes:worldAxes.visible,selection:selectionBox.visible,controls:controls.enabled,lightIntensities:state.lights.map(({light})=>light.intensity),lightVisibility:state.lights.map(({light})=>light.visible)};state.lights.forEach(({light})=>{if(light.userData.previewIntensity===undefined)light.userData.previewIntensity=light.intensity;light.intensity=light.userData.previewIntensity;light.visible=true;});lightHelpers.visible=false;grid.visible=false;worldAxes.visible=false;selectionBox.visible=false;controls.enabled=false;state.recording={renderer:recordingRenderer,camera:recordCamera,encoder,rotate:$('#rotationMode').value==='single',visibility,savedTransform,rotationOffset:new THREE.Quaternion(),fps:state.fps,totalFrames:Math.max(1,Math.round(duration/1000*state.fps))};state.recordStage='recording';dom.workspace.classList.add('recording-active');dom.frame.classList.add('visible','recording');dom.record.classList.remove('framing');dom.record.classList.add('recording');dom.record.querySelector('b').textContent=batch?'正在渲染 '+asset.name:'正在渲染';dom.record.querySelector('small').textContent='正在逐帧编码，完成后保存完整时长视频';try{const task=state.recording;await encodeRecordingFrames(task);await encoder.flush();if(encodingError.value)throw encodingError.value;muxer.finalize();return new Blob([target.buffer],{type:'video/webm'});}finally{encoder.close();dom.workspace.classList.remove('recording-active');state.recording=null;modelRoot.position.copy(savedTransform.position);modelPivotGroup.quaternion.copy(savedTransform.quaternion);modelPivotGroup.scale.copy(savedTransform.scale);updateModelBasis();state.lights.forEach(({light},index)=>{if(visibility.lightIntensities[index]!==undefined)light.intensity=visibility.lightIntensities[index];if(visibility.lightVisibility[index]!==undefined)light.visible=visibility.lightVisibility[index];});lightHelpers.visible=visibility.helpers;scene.background=previousBackground;recordingRenderer.dispose();state.transformDragging=false;grid.visible=visibility.grid;worldAxes.visible=visibility.axes;selectionBox.visible=visibility.selection;controls.enabled=true;updateContextHelpers();updateSelectionBox();refreshTransforms();renderSceneTree();renderInspector();}}
async function startSingleRecording(){if(state.recording||state.batchRunning)return;if(!(await ensureHistoryFolder()))return;if(state.recordStage==='idle'){enterFraming();return;}const asset=state.assets.find(a=>a.id===state.currentAssetId);if(!asset)return toast('请先选择模型','从右侧资产列表选择一个模型');try{const output=await createRenderOutput(asset);toast('已创建保存文件夹',output.name);const blob=await recordAsset(asset),folderName=await saveRenderedVideo(output,blob);toast('录制完成','视频已保存到 '+folderName);}catch(error){toast('录制失败',error.message);}finally{exitFraming();}}
async function startBatch(){if(state.batchRunning||state.recording)return;if(!(await ensureHistoryFolder()))return;const assets=state.assets.filter(a=>a.selected&&a.object);if(!assets.length)return toast('没有批量任务','请先勾选至少一个模型');state.batchRunning=true;dom.downloads.innerHTML='';dom.queueDone.textContent='0';dom.queueProgress.style.width='0';dom.queueStatus.textContent='渲染中';for(let index=0;index<assets.length;index++){dom.queueEta.textContent='正在处理 '+assets[index].name;try{const output=await createRenderOutput(assets[index]);const blob=await recordAsset(assets[index],true);await saveRenderedVideo(output,blob);}catch(error){console.error(error);toast('保存失败',assets[index].name+'：'+error.message);}dom.queueDone.textContent=String(index+1);dom.queueProgress.style.width=(index+1)/assets.length*100+'%';}state.batchRunning=false;dom.queueStatus.textContent='全部完成';dom.queueEta.textContent='视频已保存到历史文件夹';exitFraming();toast('批量渲染完成','已保存 '+assets.length+' 个视频');}
...
function snapView(view){const pivot=modelPivot(),bounds=modelBounds(),size=bounds.getSize(new THREE.Vector3()),maxSize=Math.max(size.x,size.y,size.z,1),distance=maxSize/(2*Math.tan(THREE.MathUtils.degToRad(camera.fov/2)))*1.28,directions={x:[0,0,1],'-x':[0,0,-1],y:[0,1,0],'-y':[0,-1,0],z:[1,0,0],'-z':[-1,0,0]},direction=new THREE.Vector3(...directions[view]).applyQuaternion(modelPivotGroup.quaternion),up=new THREE.Vector3(0,1,0).applyQuaternion(modelPivotGroup.quaternion);if(view==='y')up.set(0,0,-1).applyQuaternion(modelPivotGroup.quaternion);if(view==='-y')up.set(0,0,1).applyQuaternion(modelPivotGroup.quaternion);clearCameraPan();camera.up.copy(up);controls.target.copy(pivot);camera.position.copy(pivot).addScaledVector(direction,distance);camera.lookAt(pivot);controls.update();}
...
function setupViewportPan(){
  ...
  renderer.domElement.addEventListener('pointerdown',event=>{
    if(event.button!==2||state.recording||state.transformDragging)return;
    event.preventDefault();event.stopImmediatePropagation();
    panState={pointerId:event.pointerId,startX:event.clientX,startY:event.clientY,startPan:{...state.cameraPan}};
    renderer.domElement.setPointerCapture(event.pointerId);
  },true);
  renderer.domElement.addEventListener('pointermove',event=>{
    if(!panState||event.pointerId!==panState.pointerId)return;
    event.preventDefault();event.stopImmediatePropagation();
    state.cameraPan.x=panState.startPan.x+(event.clientX-panState.startX);
    state.cameraPan.y=panState.startPan.y+(event.clientY-panState.startY);
  ...
  const finishPan=event=>{
    ...
    panState=null;saveCurrentCameraState();
  };
  renderer.domElement.addEventListener('pointerup',finishPan,true);
  renderer.domElement.addEventListener('pointercancel',finishPan,true);
...
function animate(time){requestAnimationFrame(animate);const task=state.recording;if(!task){controls.update();state.frames++;if(time-state.fpsTime>700){const fps=$('#fpsText');if(fps)fps.textContent=`${Math.round(state.frames*1000/(time-state.fpsTime))} FPS`;state.frames=0;state.fpsTime=time;}if(state.frames%3===0&&state.previewEnabled)state.lights.forEach(({helper})=>helper?.update?.());updateViewGizmo();}if(state.previewPlaying&&!task){const duration=Number(dom.duration.value)*1000;state.previewProgress=(state.previewProgress+Math.max(0,time-state.previewLastTime)/duration)%1;state.previewLastTime=time;const base=previewTransform(),rotation=new THREE.Quaternion().setFromAxisAngle(THREE.Object3D.DEFAULT_UP,state.previewProgress*Math.PI*2);modelPivotGroup.quaternion.copy(base.quaternion).multiply(rotation);modelRoot.position.copy(base.position);modelPivotGroup.scale.copy(base.scale);updateModelBasis();updateTimelineUI();}else{state.previewLastTime=time;}if(!task)renderer.render(scene,camera);}
...
function previewTransform(){if(!state.previewStartTransform)state.previewStartTransform={position:modelRoot.position.clone(),quaternion:modelPivotGroup.quaternion.clone(),scale:modelPivotGroup.scale.clone()};return state.previewStartTransform;}
function setPreviewFrame(frame){const total=timelineFrameCount();state.previewProgress=THREE.MathUtils.clamp(frame/total,0,1);const base=previewTransform(),rotation=new THREE.Quaternion().setFromAxisAngle(THREE.Object3D.DEFAULT_UP,state.previewProgress*Math.PI*2);modelRoot.position.copy(base.position);modelPivotGroup.quaternion.copy(base.quaternion).multiply(rotation);modelPivotGroup.scale.copy(base.scale);updateModelBasis();updateTimelineUI();}
...
function updateHistoryFolderStatus(name=''){state.historyFolderName=name;dom.historyFolderStatus.textContent=name?`历史文件夹：${name}`:'未选择保存文件夹';dom.historyFolderButton.classList.toggle('selected',Boolean(name));}
async function selectHistoryFolder(){if(!('showDirectoryPicker' in window)){toast('当前环境不支持文件夹保存','请使用最新版 Chrome 或 Edge 的安全页面打开应用');return;}try{const handle=await window.showDirectoryPicker({id:'luma-render-history',mode:'readwrite',startIn:'videos'});state.historyFolderHandle=handle;updateHistoryFolderStatus(handle.name);toast('历史文件夹已选择','后续渲染会创建二级文件夹后保存');}catch(error){if(error?.name!=='AbortError')toast('无法选择历史文件夹',error.message||'请重试');}}
dom.historyFolderButton.addEventListener('click',selectHistoryFolder);
dom.historyFolderInput.addEventListener('change',event=>{event.target.value='';});
...
function modelBounds(){modelRoot.updateWorldMatrix(true,true);return new THREE.Box3().setFromObject(modelRoot);}
...
function modelPivot(){modelPivotGroup.updateWorldMatrix(true,false);return modelPivotGroup.getWorldPosition(new THREE.Vector3());}
...
function syncOrbitPivotToModel(){controls.target.copy(modelPivot());controls.update();}
